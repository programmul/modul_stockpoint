 ...
 */
class Mamoku_Multistockpoint_AccountController extends Mage_Core_Controller_Front_Action
{
    /**
    ...
    {
        $session = $this->_getSession();
         Mage::log($session, null, 'mylogfile.log');
    ...
    public function createPostAction()
    {
        ...
        try {
            ...
            if (empty($errors)) {
               ...
               Mage::log(json_encode($customer), null, 'mylogfile.log');
                $customer->save();
        ...
        $addressForm->compactData($addressData);
         ...
         $locationc = Mage::getModel('multistockpoint/locationcoverage')->getCollection();
                    ...
                    foreach ($locationc as $loc) {
                        # code...
                        if ($loc->getPropinsi() == $address['region'] && $loc->getKota() == $address['city'] && $loc->getKecamatan() == $address['kecamatan'] && $loc->getKelurahan() == $address['kelurahan']) {
                            $code = $loc->getStockpoint_code();
                        }
                    }
        
        $address['stockpointcode']=$code;
